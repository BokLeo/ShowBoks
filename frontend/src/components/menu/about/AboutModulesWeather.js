...
const AboutModulesWeather = () => {
	...
	const [requestWeatherTarget, setRequestWeatherTarget] = useState([
		'SKY',
		'T1H',
		'PTY',
		'RN1',
	...
	useEffect(() => {
		const fetchAddress = async () => {
			try {
				if (location.success) {
					// const rawPayload = await getReverseeocode(location.x, location.y); // 네이버 지도 API 유료화로 인하여 삭제
					const rawPayload = {
						data: {
							results: [],
						},
	...
	return (
		<div className='weather'>
							...
							weather.map((item, index) => {
								...
								return (
									<div key={index} className='weather-item'>
										<div className='weather-date fs-xl'>
											{item.fcstDate}
											<span className='weather-time fs-lg'>
												{' '}
												({item.fcstTime})
											</span>
										</div>
										...
										<div className='weather-data'>
											{skyStatus ? (
												<>
													<p className={`sky-icon`}>
														<AssetIcon
															iconName={SKY_CODES[skyData.fcstValue].icon}
														/>
													</p>
													<p className='sky-value fs-md'>
														{tempData.fcstValue} {tempData.unit}
													</p>
												</>
											) : (
												<>
													<p className={`sky-icon`}>
														<AssetIcon
															iconName={PTY_CODES[ptyData.fcstValue].icon}
														/>
													</p>
													<p className='sky-value fs-md'>
														{tempData.fcstValue} {tempData.unit} /{' '}
			...
			{location.success === false && (
				<p>Geolocation failed or not supported.</p>
			)}
		</div>
	);
};
